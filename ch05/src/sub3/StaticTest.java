 ...
 * 날짜 : 2025/01/08
 * 이름 : 정진우
 * 내용 : Java 클래스 변수, 클래스 메소드 실습
 * 
 * 클래스 변수, 클래스 메소드(정적 변수, 정적 메소드)
 *  - static 키워드를 선언한 변수, 메소드
 * 	- Method Area 메모리 영역에 생성
 *  - 객체를 생성하지 않고 직접 클래스 타입으로 사용
 *  - 객체들 간의 데이터 공유 목적으로 사용
 * 
 * 싱글톤 객체
 *  - static을 활용한 싱글톤 객체는 오직 하나의 인스턴스로 메모리(Method Area)에 생성
 *  - 싱글톤 객체를 사용해서 메모리 절약과 성능 향상
 *  - static 인스턴스 생성, 인스턴스 Getter 정의, private 생성자 정의(다른 클래스에서 객체를 생성하지 못하게)
 *  
 *  
 *  
 */
...
public class StaticTest {
	public static void main(String[] args) {
		...
		//c1, c2는 MethodArea에 있는 Calc를 참조, 새로운 객체가 아님.
	}
}
